...
if __name__ == "__main__":
    print("🚀 KUNLIK yuklash jarayoni boshlandi...")

    AUTH_TOKEN = "Credential Y3VzdG9tc1VzZXI6Q3UkdDBtc1BAdGh3b3Jk"
    start_date = datetime(2024, 1, 1)
    end_date = datetime.today()

    for date in day_range(start_date, end_date):
        date_str = date.strftime("%Y-%m-%d")
        print(f"\n📅 Ma'lumot yuklanmoqda: {date_str}")

        API_URL = f"http://172.16.14.21:4041/GetMinUdobDeals/{date_str}/{date_str}"
        ...
        if df is not None and not df.empty:
            ...
            print("⚠️ Bu kunda hech qanday qator topilmadi.")
